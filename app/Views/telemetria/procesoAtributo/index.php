...
<?= $this->section('content') ?>
    <h2>Registro Atributo de Proceso <?= $proceso->nombre ?></h2>
    <div class="row" >
        <div class="col text-right">
            <a href="<?= base_url('telemetria/proceso/atributo/new/'.$proceso->id) ?>" class="btn btn-success "><i class="bi bi-cloud-plus"></i> Nuevo Registro</a>
        </div>
    </div>
    <br><br>
        ...
        function Eliminar(id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= base_url('telemetria/proceso/atributo/delete');?>/"+id,          
                        type: "get",
                        dataType: "json",
                        //data: 
